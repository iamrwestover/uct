 ...
 * @ingroup themeable
 */
function uikit_av_user_admin_roles($variables) {
  $form = $variables['form'];

  $header = array(t('Role'), t('Weight'), array('data' => t(''), 'colspan' => 2));
  foreach (element_children($form['roles']) as $rid) {
    ...
    if (in_array($rid, array(DRUPAL_ANONYMOUS_RID, DRUPAL_AUTHENTICATED_RID))) {
      ...
      $row[] = '';
      $row[] = l(t('edit permissions'), 'av/users/permissions/' . $rid, array('query' => drupal_get_destination()));
    }
    else {
      $row[] = check_plain($name);
      $row[] = drupal_render($form['roles'][$rid]['weight']);
      $row[] = l(t('edit role'), 'av/users/roles/edit/' . $rid, array('query' => drupal_get_destination()));
      $row[] = l(t('edit permissions'), 'av/users/permissions/' . $rid, array('query' => drupal_get_destination()));
    }
